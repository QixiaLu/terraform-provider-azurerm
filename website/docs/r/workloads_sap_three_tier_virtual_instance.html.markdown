...
* `volume_name` - (Required) The name of the Volume. The only possible value is `default`. Possible values are `hanaData`, `hanaLog`, `hanaShared` and `usrSap`. The only possible value is `default`. Changing this forces a new resource to be created.
