...
func (s S009) Description() string {
	return "Validates that all properties exist in both schema and documentation, checks for missing properties and potential misspellings"
}
	...
	for name, schemaProperty := range schema.Objects {
		...
		if docProperty == nil {
			issue := NewValidationIssue(
				...
				s.Name(),
				fullPath,
				fmt.Sprintf("`%s` exists in schema but is missing from documentation", fullPath),
				d.Document.Path,
				"",
				"",
			)
		...
		if schemaProperty.Nested != nil && len(schemaProperty.Nested.Objects) > 0 {
			...
			if !docProperty.Block {
				issue := NewValidationIssue(
					...
					s.Name(),
					fullPath,
					fmt.Sprintf("`%s` should be declared as a block", fullPath),
					d.Document.Path,
					"",
					"",
				)
			...
			if docProperty.Nested == nil || len(docProperty.Nested.Objects) == 0 {
				...
				errs = append(errs, fmt.Errorf("S009: `%s` block is missing nested properties", fullPath))
				continue
			}
	...
	for name, docProperty := range documentation.Objects {
		...
		if len(docProperty.ParseErrors) > 0 {
			for _, parseErr := range docProperty.ParseErrors {
				if strings.Contains(parseErr, "misspell of name from") {
					errs = append(errs, fmt.Errorf("S009: `%s` does not exist in schema - possible misspelling?", fullPath))
				}
			}
			continue
		}
		...
		if schemaProperty == nil {
			issue := NewValidationIssue(
				...
				s.Name(),
				fullPath,
				fmt.Sprintf("`%s` exists in documentation but not in schema", fullPath),
				d.Document.Path,
				"",
				"",
			)
